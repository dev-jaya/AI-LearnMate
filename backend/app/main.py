...
app = FastAPI(title="AI LearnMate API", version="1.0.0")
app.add_middleware(CORSMiddleware, allow_origins=["http://localhost:5173", "http://localhost:5500", "http://127.0.0.1:5173", "http://127.0.0.1:5500"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])
...
@app.get("/")
def root(): return {"name":"AI LearnMate","status":"running"}
...
async def generate_quiz(req:QuizRequest, db:Session):
    ...
    for _ in range(3):
        ...
        for question in candidates or []:
            if question["fingerprint"] in excluded or question["fingerprint"] in {item["fingerprint"] for item in questions}: continue
            if any(question_similarity(question["question"], previous) >= 0.9 for previous in excluded_questions + [item["question"] for item in questions]): continue
...
def submit(req:SubmitRequest, db:Session=Depends(get_db)):
    ...
    if req.quiz_id:
        ...
        if any(answer not in range(4) for answer in req.answers): raise HTTPException(400, "Answers must use option indexes 0 to 3")
        correct = sum(answer == question["answer"] for answer, question in zip(req.answers, questions))
        total = len(questions)
        topic, difficulty = quiz.topic, quiz.difficulty
        quiz.submitted = True
        ...
        raise HTTPException(400, "Quiz id is required")
    score=round(correct*100/total,2)
    a=Attempt(learner_id=req.learner_id,topic=topic,difficulty=difficulty,score=score,total=total,answers_json=json.dumps(req.answers))
    db.add(a); db.commit(); db.refresh(a)
    return {"id":a.id,"topic":a.topic,"score":a.score,"total":a.total,"difficulty":a.difficulty,"created_at":a.created_at.isoformat()}

@app.post("/api/assessment/start", response_model=QuizOut)
async def start_assessment(req:QuizRequest, db:Session=Depends(get_db)):
    return await create_quiz(req, db)
...
def dashboard(learner_id:int, db:Session=Depends(get_db)):
    ...
    mastery={k:round(sum(v)/len(v),1) for k,v in topic_scores.items()}
    weak=sorted(mastery.items(), key=lambda x:x[1])[:3]
    recommendations=[]
    for topic,score in weak:
        if score<70: recommendations.append({"topic":topic,"reason":f"Mastery is {score}%. Practice this topic next.","priority":"high"})
    if not recommendations: recommendations=[{"topic":"Python","reason":"Start a diagnostic assessment to build your learning profile.","priority":"medium"}]
...
def tutor_context(learner_id:int, topic:str, db:Session):
    learner = db.get(Learner, learner_id)
    if not learner: raise HTTPException(404, "Learner not found")
    ...
    learning_path = [{"topic": name, "mastery": mastery.get(name), "action": "revision" if mastery.get(name, 0) < 50 else "focused practice" if mastery.get(name, 0) < 80 else "advanced practice"} for name in weak + strong]
    return {"learner_name": learner.name, "topic": topic, "mastery": mastery, "weak_topics": weak, "strong_topics": strong, "learning_path": learning_path, "recent_results": [{"topic": a.topic, "score": a.score, "difficulty": a.difficulty} for a in recent], "recent_mistakes": [item.subtopic for item in mistake_questions if item], "recent_questions": [q.question for q in recent_questions], "recent_question_details": [{"question": q.question, "explanation": q.explanation, "subtopic": q.subtopic} for q in recent_questions], "recent_question_types": [q.question_type for q in recent_questions], "recent_quiz_difficulty": recent_quiz.difficulty if recent_quiz else None}
...
async def chat(req:ChatRequest, db:Session=Depends(get_db)):
    ...
    context = tutor_context(req.learner_id, topic, db)
    ...
    quiz_intent = any(phrase in lower for phrase in ("test me", "quiz me", "mcq", "ask me", "give me questions", "give me  questions", "create a quiz", "generate questions", "make them harder", "make them easier", "make them intermediate")) or ("another" in lower and bool(re.search(r"\b\d+\b", lower)))
    if quiz_intent:
        ...
        if "weak" in lower or "weak topics" in lower:
            selected_topic = context["weak_topics"][0] if context["weak_topics"] else selected_topic
        if any(term in lower for term in ("hard", "difficult")):
            difficulty = "hard"
        elif any(term in lower for term in ("easy", "easier", "beginner")):
            difficulty = "easy"
        elif any(term in lower for term in ("medium", "intermediate")):
            difficulty = "medium"
        else:
            difficulty = context.get("recent_quiz_difficulty") if "another" in lower else "adaptive"
        ...
        reply = f"I prepared {count} new {selected_topic} questions at {quiz['difficulty']} difficulty. They are selected from your learner history and avoid previously shown fingerprints."
        ...
        if provider_reply:
            ...
            reply = await fallback.chat(req.message, context) or "I could not reach the configured AI provider. I can still generate local adaptive MCQs and explain the core concepts."
