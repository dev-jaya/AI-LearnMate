...
from .models import (
    Learner, Attempt, Quiz, Conversation, Message, GeneratedQuestion,
    QuestionFingerprint, LearnerQuestionHistory, Subject, Topic, QuizQuestion,
)
...
@app.get("/health/ai")
def ai_health():
    provider = OpenAIResponsesProvider() if _openai_configured() else get_provider()
    return {
        "provider": provider.name,
        "model": getattr(provider, "model", None),
        "key_loaded": bool(getattr(provider, "api_key", "")),
    }
...
def topics(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "subjects": [{"name": record.name, "topics": [item.name for item in record.topics if item.active]} for record in records],
    }
...
async def generate_quiz(req: QuizRequest, db: Session):
    ...
    excluded_questions = [
        row.question for row in db.query(GeneratedQuestion)
        .filter(GeneratedQuestion.id.in_(question_ids), GeneratedQuestion.topic == req.topic)
        .order_by(GeneratedQuestion.created_at.desc()).limit(30).all()
    ] if question_ids else []
    provider = get_provider()
    provider_failures = 0
    context = {
        "mastery": mastery,
        "weak_topics": [name for name, score in mastery.items() if score < 70],
        "strong_topics": [name for name, score in mastery.items() if score >= 80],
    ...
    for _ in range(3):
        ...
        candidates = await provider.generate_questions(
            subject, req.topic, req.subtopic, difficulty, max(remaining * 2, 6), context
        )
...
def submit(req: SubmitRequest, db: Session = Depends(get_db)):
    ...
    for answer, question in zip(req.answers, questions):
        ...
        if stored:
            history = db.query(LearnerQuestionHistory).filter(
                LearnerQuestionHistory.learner_id == req.learner_id,
                LearnerQuestionHistory.question_id == stored.id,
            ).order_by(LearnerQuestionHistory.seen_at.desc()).first()
            if history:
                history.answered = True
                history.correct = answer == question["answer"]
    score = round(correct * 100 / total, 2)
    attempt = Attempt(
        learner_id=req.learner_id, topic=quiz.topic, difficulty=quiz.difficulty,
        score=score, total=total, answers_json=json.dumps(req.answers),
    )
    db.add(attempt)
    db.commit()
    db.refresh(attempt)
    return {
        "id": attempt.id, "topic": attempt.topic, "score": attempt.score,
        "total": attempt.total, "difficulty": attempt.difficulty,
...
def dashboard(learner_id: int, db: Session = Depends(get_db)):
    ...
    recommendations = [
        {"topic": topic, "reason": f"Mastery is {score}%. Practice this topic next.", "priority": "high"}
        for topic, score in weak if score < 70
    ]
    ...
    learning_path = [
        {"topic": topic, "mastery": mastery.get(topic), "action": "Advanced practice" if mastery.get(topic, 0) >= 80 else "Revision and guided practice" if mastery.get(topic, 0) < 50 else "Focused practice"}
    ...
    return {
        ...
        "mastery": mastery, "weak_topics": weak_topics, "strong_topics": strong_topics,
        "learning_path": learning_path, "recommendations": recommendations,
        "recent": [{"topic": a.topic, "score": a.score, "difficulty": a.difficulty, "date": a.created_at.isoformat()} for a in attempts[:8]],
    }
...
async def upload_material(learner_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    material = Material(
        learner_id=learner_id, title=(file.filename or "Learning material")[:200],
        filename=file.filename or "material", mime_type=file.content_type or "application/octet-stream",
        source_type="upload", extracted_text=text_content,
    )
    db.add(material)
    db.commit()
    db.refresh(material)
...
async def import_igot(learner_id: int, url: str, db: Session = Depends(get_db)):
    ...
    material = Material(
        learner_id=learner_id, title=title, filename=title, mime_type=content_type,
        source_type="igot", source_url=url, extracted_text=text_content,
    )
    db.add(material)
    db.commit()
    db.refresh(material)
...
def list_materials(learner_id: int, db: Session = Depends(get_db)):
    ...
    return {
        "materials": [{
            ...
            "characters": len(item.extracted_text), "created_at": item.created_at.isoformat(),
        } for item in materials]
    }


async def _generate_material_quiz(material: Material, req: QuizRequest, db: Session):
    """Single provider-neutral material quiz path. No Gemini-specific gate."""
    provider = OpenAIResponsesProvider() if _openai_configured() else get_provider()
    if not hasattr(provider, "generate_material_questions"):
        raise HTTPException(503, "The configured AI provider cannot generate material-grounded questions.")
    mastery = learner_mastery(req.learner_id, db)
    difficulty = req.difficulty
    if difficulty == "adaptive":
        score = mastery.get(req.topic)
    ...
    previous = [
        row.question for row in db.query(GeneratedQuestion)
        .filter(GeneratedQuestion.learner_id == req.learner_id, GeneratedQuestion.topic == req.topic)
        .order_by(GeneratedQuestion.created_at.desc()).limit(30).all()
    ]
    ...
    for _ in range(3):
        ...
        candidates = await provider.generate_material_questions(
            source, req.subject or req.topic or material.title,
            req.topic or material.title, difficulty, max(remaining * 2, 6), excluded,
        )
        for question in candidates or []:
            ...
            if len(questions) == req.count:
                break
        excluded = previous + [item["question"] for item in questions]
        if len(questions) == req.count:
            break
    if len(questions) < req.count:
        raise HTTPException(502, "The configured AI provider could not produce enough new material-grounded questions. Try a shorter or clearer source document.")
...
def tutor_context(learner_id: int, topic: str, db: Session, material_id: int | None = None):
    ...
    mistakes = db.query(LearnerQuestionHistory).filter(
        LearnerQuestionHistory.learner_id == learner_id,
        LearnerQuestionHistory.correct.is_(False),
    ).order_by(LearnerQuestionHistory.seen_at.desc()).limit(5).all()
    mistake_questions = [db.get(GeneratedQuestion, item.question_id) for item in mistakes]
    context = {
        "learner_name": learner.name, "topic": topic, "mastery": mastery,
        "weak_topics": weak, "strong_topics": strong,
        "learning_path": [{
            "topic": name, "mastery": mastery.get(name),
            "action": "revision" if mastery.get(name, 0) < 50 else "focused practice" if mastery.get(name, 0) < 80 else "advanced practice",
        } for name in weak + strong],
        "recent_results": [{"topic": a.topic, "score": a.score, "difficulty": a.difficulty} for a in recent],
        "recent_mistakes": [item.subtopic for item in mistake_questions if item],
        "recent_questions": [q.question for q in recent_questions],
        "recent_question_details": [{"question": q.question, "explanation": q.explanation, "subtopic": q.subtopic} for q in recent_questions],
        "recent_question_types": [q.question_type for q in recent_questions],
        "recent_quiz_difficulty": recent_quiz.difficulty if recent_quiz else None,
    }
    if material_id:
        material = db.get(Material, material_id)
...
def list_conversations(learner_id: int, db: Session = Depends(get_db)):
    if not db.get(Learner, learner_id):
        raise HTTPException(404, "Learner not found")
    conversations = db.query(Conversation).filter(Conversation.learner_id == learner_id).order_by(Conversation.updated_at.desc(), Conversation.created_at.desc()).all()
    result = []
    for conversation in conversations:
        first_user = db.query(Message).filter(Message.conversation_id == conversation.id, Message.role == "user").order_by(Message.created_at.asc()).first()
        last_message = db.query(Message).filter(Message.conversation_id == conversation.id).order_by(Message.created_at.desc()).first()
        result.append({
            "id": conversation.id,
            "topic": conversation.topic,
            "title": (first_user.content[:70] if first_user else conversation.topic or "New conversation"),
...
async def chat(req: ChatRequest, db: Session = Depends(get_db)):
    ...
    quiz_intent = any(
        phrase in lower for phrase in (
            "test me", "quiz me", "mcq", "ask me", "give me questions", "create a quiz",
            "generate questions", "make them harder", "make them easier", "make them intermediate",
        )
    ) or ("another" in lower and bool(re.search(r"\b\d+\b", lower)))
    ...
    if quiz_intent:
        ...
        difficulty = (
            "hard" if any(term in lower for term in ("hard", "difficult"))
            else "easy" if any(term in lower for term in ("easy", "easier", "beginner"))
            else "medium" if any(term in lower for term in ("medium", "intermediate"))
            else (context.get("recent_quiz_difficulty") if "another" in lower else "adaptive") or "adaptive"
        )
        quiz_request = QuizRequest(
            learner_id=req.learner_id, topic=selected_topic, subject=selected_topic,
        ...
        if req.material_id:
            ...
            if not material or material.learner_id != req.learner_id:
                raise HTTPException(404, "Learning material not found")
            quiz = await _generate_material_quiz(material, quiz_request, db)
        else:
            quiz = await generate_quiz(quiz_request, db)
        reply = f"I prepared {count} new {selected_topic} questions at {quiz['difficulty']} difficulty." + (
            " They are grounded in your selected learning material." if req.material_id else " They avoid previously shown questions."
        )
        ...
        if not provider_reply:
            if _openai_configured():
                detail = getattr(provider, "last_error", "unknown provider error")
                raise HTTPException(503, f"The configured AI provider is unavailable right now: {detail}")
            fallback = FallbackProvider()
            provider_reply = await fallback.chat(req.message, context)
            response_provider = fallback.name
        else:
            response_provider = provider.name
        reply = provider_reply or "I could not generate a response. Please try again."
    ...
    return {
        ...
        "message": {"id": assistant.id, "role": assistant.role, "content": assistant.content, "created_at": assistant.created_at.isoformat()},
        "quiz": quiz,
